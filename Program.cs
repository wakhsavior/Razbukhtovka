...
void Perestanovka(int[] array, int i, int n, ulong summ)
{

    int[][] variants = new int[summ][];
    int j = 0;
    if (i > 0)
    {
        j = array[i - 1] + 1; ;
    ...
    while (j < n)
    {
        ...
        if (i + 1 < length)
        {
            Perestanovka(array, i + 1, n, summ);
        }
        else
        {
            // variants[arrayIndex] = new int[i];
            // variants[arrayIndex] = array;
            // Console.Write($"{arrayIndex} - ");
            PrintArray(array);
            // arrayIndex++;
            Thread.Sleep(200);
        }
...
int[][] variants = new int[summOptions][];

int arrayIndex = 0;
for (int i = 1; i <= n; i++)
{
    int[] array = new int[i];
    Perestanovka(array, 0, n, summOptions);
